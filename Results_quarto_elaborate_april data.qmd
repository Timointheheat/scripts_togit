...
Masterfile <- read_excel(paste0(here("data/data_output"), "/HA_Masterfile_cleaned_17042025.xlsx")) 
...
Masterfile <- Masterfile %>%
  mutate(T_core_torso_new = T_core_torso_consumer,
         T_core_hand_new = T_core_hand_consumer)


# STEP 2) Answer if CORE can be used to track HA-adaptations ----

## 0) exclude people who did not finish HA-protocol or Trec measurement error
...
long_Masterfile %>%
  ...
  stat_summary(fun = mean, geom = "line", aes(group = device), size = 2) +
  stat_summary(fun.data = mean_cl_normal, geom = "ribbon", aes(fill = device), alpha = 0.5, color = NA)  +
  publication_theme() +
